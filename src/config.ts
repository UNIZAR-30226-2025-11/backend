...
export const INITIAL_HAND_SIZE: number = parseInt(process.env.INITIAL_HAND_SIZE || "6", 10);

// console.log(`Using initial hand size: ${INITIAL_HAND_SIZE}`);

export const TURN_TIME_LIMIT: number = parseInt(process.env.TURN_TIME_LIMIT || "10000", 10); 
export const TIMEOUT_RESPONSE: number = parseInt(process.env.TIMEOUT_RESPONSE || "10000", 10);
...
export const CARD_COUNTS: { [key in CardType]: number } = {
    ...
    [CardType.HairyPotatoCat]: parseInt(process.env.CARD_HAIRY_POTATO_CAT || "5", 10),
...
const totalCardCount = Object.values(CARD_COUNTS).reduce((sum, count) => sum + count, 0);
...
export const EXTRA_BOMBS: number = parseInt(process.env.CARD_BOMB_EXTRA || "0", 10);
export const EXTRA_DEACTIVATES: number = parseInt(process.env.CARD_DEACTIVATE_EXTRA || "0", 10);
