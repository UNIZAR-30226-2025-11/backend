...
class Terminal extends CallSystem {
    ...
    notify_current_hand(player: Player): void {
        console.log(`Player ${player.id} has cards: ${player.hand.toString()}`);
    }

    notify_hidden_cards(cards: CardArray, player:Player): void {
        console.log(`Player ${player.id} can see the cards: ${cards.toString()}`);
    }

    notify_new_cards(player:Player): void {
        console.log(`Player ${player.id} has new cards: ${player.hand.toString()}`);
    }

    notify_bomb_defused(player: Player): void {
        console.log(`Player ${player.id} has defused the bomb with a deactivation card!`);
    }

    notify_attack(player: Player, type_attack:AttackType): void {
        console.log(`Player ${player.id} has attacked with a ${AttackType[type_attack]} attack!`);
    }

    notify_attack_result(attacked_player: Player, current_player: Player, type_attack: AttackType, result: boolean): void {
        if (result) {
            console.log(`Player ${attacked_player.id} has been attacked by player ${current_player.id} with a ${AttackType[type_attack]} attack!`);
            console.log(`Player ${attacked_player.id} has lost the round!`);
        } else {
            console.log(`Player ${attacked_player.id} has been attacked by player ${current_player.id} with a ${AttackType[type_attack]} attack!`);
            console.log(`Player ${attacked_player.id} has defended the round!`);
        }
    }
